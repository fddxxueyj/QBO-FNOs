...
Structure:
1. `critical_layer.ipynb`: Physical understanding for 1D-QBO model, mainly concerning critical layer;
2. `param_kappa.ipynb` and `param_alpha.ipynb`: Parameter sensitivity analysis in 1D-QBO model, including the thermal dissipation rate of waves $\alpha$ and the vertical diffusion of the mean flow $\kappa$;
3. `MLP.ipynb`: MLPs (containing online test);
4. `FNO.ipynb`: FNOs (containing online test);
5. `presentation.pdf`: Final report and the presentation for the defense.
